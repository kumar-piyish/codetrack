...
export default function Profile() {
  ...
  const getStats = () => {
    if (leetcodeData.stats) {
      return {
        easy: {
          solved: leetcodeData.stats.easySolved || 0,
          color: "text-teal-500",
        },
        medium: {
          solved: leetcodeData.stats.mediumSolved || 0,
          color: "text-yellow-500",
        },
        hard: {
          solved: leetcodeData.stats.hardSolved || 0,
          color: "text-red-500",
        },
      };
    }
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.data) {
          ...
          if (
            response.data.googleCalendar?.connected &&
  ...
  const onboardingSteps = [
    ...
    {
      title: "Get your Revision Questions of the Day and Weekly Insights",
      description: "",
      hint: "On Email Notifications and Sync your Google Calendar to get your Revision Questions of the Day and Weekly Insights",
    },
  ];
  ...
  const handleSyncLeetCode = async () => {
    ...
    try {
      const response = await api.post(`/api/user/${user.id}/leetcode/sync`, {
  ...
  const handleSyncCalendar = async () => {
    ...
    try {
      ...
      setCalendarStatus(
        err?.response?.data?.error || "Failed to sync calendar.",
      );
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50">
      {showOnboarding && (
        <div className="fixed inset-0 z-50 bg-black/40 hidden lg:block">
          ...
          {onboardingStep === 4 && (
            <div className="fixed top-100 right-150 z-[60] hidden w-80 rounded-2xl bg-white p-5 shadow-2xl md:block">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase text-blue-600">
                    Quick Guide
                  ...
                  <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:gap-6">
                    ...
                    <div className="relative shrink-0">
                      ...
                      >
                        ...
                        <circle
                          ...
                          strokeDashoffset={
                            totalSolved > 0 ? offset * 0.8 : circumference * 0.8
                          }
                          strokeLinecap="round"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <p className="text-2xl font-bold sm:text-3xl mr-2">
                          {totalSolved}
                        </p>
                    ...
                    <div className="w-full min-w-[180px] space-y-2 sm:w-auto">
                      {Object.entries(stats).map(([key, value]) => (
                        ...
                        >
                          <span
                            className={`font-medium capitalize ${value.color}`}
                          >
                            {key}
                          </span>
                          <span className="text-gray-300">{value.solved}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    {isEditing ? (
                      ...
                      <div className={`rounded-lg bg-blue-600 p-4`}>
                        <span className="text-lg font-bold text-white">
                          {profileData.currentLevel.charAt(0).toUpperCase() +
                            profileData.currentLevel.slice(1)}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <div className="flex flex-wrap gap-3">
                        {platforms
                          .filter((p) =>
                            profileData.targetPlatform.includes(p.name),
                          )
                          .map((platform) => (
                            <div
                              key={platform.name}
                    ...
                    {isEditing ? (
                      ...
                      <div className="rounded-lg px-4">
                        <p className="text-3xl font-bold ">
                          {profileData.dailyPractice || "0"} hour
                          {profileData.dailyPractice !== "1" ? "s" : ""}
                        </p>
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <label className="relative inline-flex cursor-pointer items-center">
                        ...
                        />
                        <div
                          className={`h-6 w-11 rounded-full ${profileData.emailNotification ? "bg-blue-500" : "bg-gray-300"}`}
                        >
                          <div
                            className={`absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${profileData.emailNotification ? "translate-x-6" : "translate-x-0.5"}`}
                          />
                        </div>
                      </label>
                    </div>
                      ...
                      {profileData.googleCalendar?.connected ? (
                        <>
                          ...
                          <span className="text-xs text-gray-600">
                            {profileData.googleCalendar?.email ||
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  {[
                    ...
                    },
                    {
                      name: "twitter",
                      label: "Twitter",
                      icon: "https://img.freepik.com/premium-vector/instagram-vector-logo-icon-social-media-logotype_901408-392.jpg?semt=ais_hybrid&w=740&q=80",
                      color: "bg-sky-500 text-white",
                    },
                    {
                      name: "insta",
                      label: "Instagram",
                      icon: "https://img.freepik.com/free-vector/new-2023-twitter-logo-x-icon-design_1017-45418.jpg?semt=ais_hybrid&w=740&q=80",
                ...
                <div>
                    ...
                    {isEditing && profileData.leetcodeUsername && (
                      ...
                      >
                        <RefreshCw
                          size={16}
                          className={
                            leetcodeData.isSyncing ? "animate-spin" : ""
                          }
                        />
                  ...
                  {!isEditing && !profileData.leetcodeUsername && (
                    <p className="mt-2 text-sm text-gray-500">
                      Add your LeetCode username to sync your solved problems
                      and streak
                    </p>
                  )}
                </div>
                  ...
                  <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <div className="flex h-full flex-col rounded-xl  bg-blue-50 border border-blue-600 p-5 text-blue-600">
                      ...
                      <ul className="mt-3 space-y-2 text-sm">
                        <li>
                          ✓ Track 50 questions and Get reminders through
                          calendar
                        </li>
                        <li>✓ confidence insights</li>
                        <li>✓ 40 company sheets </li>
                      </ul>
                    ...
                    <div className="flex h-full flex-col rounded-xl border border-gray-500 bg-white p-5 text-gray-700">
                      <div>
                        ...
                        <p className="text-sm text-black">₹199/mon.</p>
                      </div>
                      <p className="mt-4 text-sm text-black">
                        Everything in Hobby, plus:
                      </p>
                      <ul className="mt-3 space-y-2 text-sm">
                        <li>✓ Get 40 more company sheets</li>
